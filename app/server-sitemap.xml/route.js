...
import { designers, getAllWorks, getDesignerByWorkId } from '../../data/designers';

export async function GET(request) {
  const baseUrl = process.env.SITE_URL ? process.env.SITE_URL.replace(/"/g, '') : 'https://www.scid110.com';
  ...
  try {
    ...
    const categoryPaths = categories.map(category => {
      ...
      let categorySlug;
      switch(category) {
        case "舒適巢": categorySlug = "comfortable-nest"; break;
        case "溫工藝": categorySlug = "warm-craft"; break;
        case "熱對話": categorySlug = "hot-dialogue"; break;
        case "冷火花": categorySlug = "cold-spark"; break;
        default: categorySlug = encodeURIComponent(category);
      }
